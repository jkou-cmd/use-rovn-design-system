...
3. Parse input using `node scripts/render.mjs INPUT --prepare output/working/document.rovn.json`. Preparation embeds document images, so the JSON can be relocated without breaking relative paths. Approved amber asset paths remain repository-relative.
...
- The cover is unnumbered; printed page 1 is the ToC. The ToC and appendix artwork is fixed. Cover and section images may use only `assets/bg1.png` through `assets/bg4.png`.
...
- Decorative imagery may come from this repository even if the source contains no images. Do not use outside or generated imagery unless the user requests it. Preserve captions and alt text from source images. Do not caption decorative imagery as factual evidence.
